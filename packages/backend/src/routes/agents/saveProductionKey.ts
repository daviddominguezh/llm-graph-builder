...
import { updateProductionKeyId } from '../../db/queries/agentQueries.js';
import {
...
export async function handleSaveProductionKey(req: Request, res: AuthenticatedResponse): Promise<void> {
  ...
  try {
    const { error } = await updateProductionKeyId(supabase, agentId, keyId);
