...
import { getTenantSummary } from '../../db/queries/dashboardQueries.js';
import {
...
export async function handleGetTenantSummary(req: Request, res: AuthenticatedResponse): Promise<void> {
  ...
  try {
    const result = await getTenantSummary(supabase, orgId, params);
