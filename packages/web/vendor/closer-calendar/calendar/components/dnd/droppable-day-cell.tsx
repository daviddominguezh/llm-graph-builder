...
import { ItemTypes } from "@cc/calendar/components/dnd/draggable-event";

import type { IEvent, ICalendarCell } from "@cc/calendar/interfaces";
...
export function DroppableDayCell({ cell, children }: DroppableDayCellProps) {
  ...
  return (
    <div ref={drop as unknown as React.RefObject<HTMLDivElement>} className={cn(isOver && canDrop && "twcal:bg-accent/50")}>
      {children}
    </div>
  );
}
