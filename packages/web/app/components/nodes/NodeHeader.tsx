import { Button } from '@/components/ui/button';
import { ListTodo, Play, Shrink, Split, UserRoundPen } from 'lucide-react';
import { memo } from 'react';

import type { NodeKind } from '../../utils/nodeKind';
import { useHandleContext } from './HandleContext';

interface NodeHeaderProps {
  nodeKind: NodeKind;
  agent?: string;
  nodeId: string;
}

const NodeHeaderComponent = ({ nodeKind, nodeId }: NodeHeaderProps) => {
  const { onZoomToNode } = useHandleContext();
  let headerLabel: string;
  let headerIcon: React.ReactNode;
  let colorClass: string;

  const iconClass = 'h-3.5 w-3.5 text-white';

  switch (nodeKind) {
    case 'user_routing':
      headerLabel = 'User Input';
      colorClass = 'bg-green-600';
      headerIcon = <UserRoundPen className={iconClass} />;
      break;
    case 'agent_decision':
      headerLabel = 'LLM Decision';
      colorClass = 'bg-purple-600';
      headerIcon = <Split className={iconClass} />;
      break;
    case 'tool_call':
      headerLabel = 'Tool call';
      colorClass = 'bg-orange-600';
      headerIcon = <Play className={iconClass} />;
      break;
    default:
      headerLabel = 'Execution';
      colorClass = 'bg-muted-foreground';
      headerIcon = <ListTodo className={iconClass} />;
  }

  return (
    <div className="shrink-0 px-1.5 py-1 flex justify-between items-center group">
      <div className="flex items-center gap-2 p-0">
        <div className={`${colorClass} rounded-sm flex items-center justify-center p-1.5`}>{headerIcon}</div>
        <div className="flex flex-col gap-0">
          <span className={`text-[9px] uppercase text-foreground/55`}>{headerLabel}</span>
          <p className="text-[10px] uppercase font-semibold text-foreground">{nodeId}</p>
        </div>
      </div>

      <Button
        variant="ghost"
        size="icon-sm"
        className="mr-0 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity rounded-sm"
        onClick={(e) => {
          e.stopPropagation();
          onZoomToNode?.(nodeId);
        }}
      >
        <Shrink className="h-3 w-3" />
      </Button>
    </div>
  );
};

export const NodeHeader = memo(
  NodeHeaderComponent,
  (prev, next) => prev.nodeKind === next.nodeKind && prev.agent === next.agent && prev.nodeId === next.nodeId
);
