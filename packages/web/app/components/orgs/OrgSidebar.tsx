...
export function OrgSidebar({ org }: OrgSidebarProps) {
  ...
  return (
    <aside
      className={`absolute left-0 top-0 bottom-0 z-11 flex flex-col gap-2 overflow-hidden p-2 pl-1.5 transition-[width,background-color] duration-100 ${sidebar.collapsed ? 'w-[52px] bg-sidebar' : 'w-58.5 bg-background'} ${sidebar.contentCollapsed ? 'border border-transparent' : 'shadow-lg border rounded-e-md z-12'}`}
