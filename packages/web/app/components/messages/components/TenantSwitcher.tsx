...
function SingleTenantLabel({ name }: { name: string }) {
  return (
...
function MultiTenantTriggerContent({ name }: { name: string }) {
  return (
...
export function TenantSwitcher({ tenants, currentTenantId, onTenantChange }: TenantSwitcherProps) {
  ...
  if (tenants.length <= 1) {
    return <SingleTenantLabel name={currentName} />;
  }

  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger className="w-full cursor-pointer">
        <MultiTenantTriggerContent name={currentName} />
      </DropdownMenuTrigger>
