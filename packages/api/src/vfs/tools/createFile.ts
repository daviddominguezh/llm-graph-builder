...
import type { CreateFileInput } from './schemas.js';
...
export function createCreateFileTool(vfs: VFSContext): Tool<CreateFileInput> {
  return tool({
    description: 'Create a new file. Fails if the file already exists.',
    inputSchema: CreateFileSchema,
    execute: async (data: CreateFileInput, { toolCallId }) => {
      try {
