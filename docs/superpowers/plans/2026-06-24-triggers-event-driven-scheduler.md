...
```
Add a handler test (all deps injected as fakes — no DB): fake `claimAndRearm` returns `{ runId, sessionId }`; assert the handler (a) for a **real-fire** payload (`hopEpoch === targetEpoch`) calls `claimAndRearm` **before** `scheduler.schedule`, arms the next occurrence, and responds `202`; (b) when `claimAndRearm` returns `null` (disabled/duplicate) responds `200` and **does not** call `scheduler.schedule` or `execute`; (c) for a **continuation** payload (`hopEpoch !== targetEpoch`) it does **not** claim or run — when `isTriggerEnabled` is true it `armToward`s the target and responds `200` (`continuation: true`), and when false it arms nothing. Use fakes for `execute`, `getAgentById`, `isTriggerEnabled`, and the scheduler. Also unit-test `armToward` directly: target within `horizonMs` → `hopEpoch === targetEpoch` (real fire); target beyond `horizonMs` → `hopEpoch < targetEpoch` at `now + horizonMs` (hop), and `setArmedTaskEpoch` is called with the chosen `hopEpoch`.
...
`createTrigger.ts`: `safeParse` the body (`z.object({ tenantId: z.string().min(1), schedule: TriggerScheduleSchema, initialMessage: z.string().min(1) })`). **App-generate the id** (`crypto.randomUUID()`) so jitter can be seeded before the insert and we write once: build the flat schedule input from the validated union, `const next = nextRunFor(scheduleInput, new Date(), id, jitterWindowMs)`, then a **single** `insertTrigger({ id, agentId, tenantId, orgId, …schedule columns…, initialMessage, nextRunAt: next })`, then (when `next !== null`) **`armToward({ supabase, scheduler: getTriggerScheduler(), horizonMs }, { triggerId: id, agentId, tenantId, initialMessage, schedule: scheduleInput }, floor(next/1000), new Date())`** — which arms the first task (a hop if the target is >horizon out, e.g. monthly) and records `armed_task_epoch`. Return the `TriggerRow`. `listTriggers.ts`/`deleteTrigger.ts`/`setTriggerEnabled.ts` follow the handler-per-file style (each ≤40 lines). `deleteTrigger`/`setTriggerEnabled(disable)` cancel the armed task with **`getTriggerScheduler().cancel({ triggerId, taskEpoch: row.armed_task_epoch })`** (skip if null); `setTriggerEnabled(enable)` recomputes `next_run_at` and calls `armToward(...)`. Import `getTriggerScheduler()` (Task C1, Step 6) and `armToward` (Task C3 `fireHelpers`); `horizonMs` from `TRIGGER_MAX_HORIZON_MS`. `triggerRouter.ts`:
```ts
...
const HORIZON = Number(process.env.TRIGGER_MAX_HORIZON_MS ?? String(20 * 24 * 60 * 60 * 1000)); // 20 days < both ceilings
internalRouter.post('/triggers/fire', createFireHandler({
  supabase: createServiceClient(), scheduler: getTriggerScheduler(),
  jitterWindowMs: JITTER, defaultUserId: DEFAULT_USER, execTimeoutMs: EXEC_TIMEOUT, horizonMs: HORIZON,
}));
```
...
- **Deferred (out of scope, per spec):** min-interval, per-org/agent caps, per-org budget/concurrency, after-event firing, reconciliation sweep, OIDC hardening.
- **Long inter-occurrence gaps (built):** monthly recurring, ≥4-week intervals, and far-future `once` are handled by the **continuation hop** — `armToward` arms an intermediate task at `now + TRIGGER_MAX_HORIZON_MS` (~20 days, under both the Cloud Tasks 30-day and `setTimeout` ~24.8-day ceilings) that re-arms toward the target without running, until within horizon. Adapter-agnostic (works identically in prod/Cloud Tasks and localhost/timer); `armed_task_epoch` records the live hop for exact cancel; the continuation branch re-checks `enabled` so a paused trigger's stray hop stops the chain.
