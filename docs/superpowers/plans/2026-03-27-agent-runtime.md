...
function appendToolMessages(
  ...
  for (const tc of stepResult.toolCalls) {
    state.messages.push({ role: 'assistant', content: [{ type: 'tool-call', toolCallId: tc.toolName, toolName: tc.toolName, args: tc.input }] });
    state.messages.push({ role: 'tool', content: [{ type: 'tool-result', toolCallId: tc.toolName, toolName: tc.toolName, result: tc.output }] });
...
async function runLoop(
  ...
  if (stepResult.done) {
    return buildLoopResult(stepResult.text, state.step, state.totalTokens, state.tokensLogs, state.allToolCalls);
  }
...
**Note:** The `appendToolMessages` function constructs proper Vercel AI SDK message types. When the tools are MCP tools wired through the `ai` SDK, `generateText` automatically executes tool calls and appends results. However, the loop captures tool call data for callbacks/persistence. The exact message format may need adjustment during integration to match the `ai` SDK's expected `ModelMessage` types for tool-call/tool-result pairs. Adjust the `role` and `content` structures to match Vercel AI SDK conventions (`AssistantModelMessage` with tool-call parts, `ToolModelMessage` with tool-result parts).
...
git commit -m "feat: add agent execution loop with recursive step runner"
```
...
function isFlushable(value: unknown): value is Flushable {
  return typeof value === 'object' && value !== null && 'flush' in value;
}
...
In `executeFetcher.ts`, add `appType: string` to `FetchedData`:
...
In `fetchAllData` in `executeHandler.ts`, propagate `appType`:

```ts
return { ...graphAndKeys, ...sessionData, graph: resolvedGraph, appType: graphAndKeys.appType };
```

- [ ] **Step 2: Add agent loop execution path to executeHandler**

In `executeHandler.ts`, create a new function for the agent execution path.

Create an `executeAgentPath` helper that calls `executeAgentLoop` with agent-specific fields from the fetched data. The agent's `system_prompt` and context come from the version snapshot (stored in the graph data under `agentConfig` or from the agents table). For now, use the graph's `systemPrompt` field if present, or fall back to an empty string.

Add these helper functions in `executeHandler.ts`:
...
function extractAgentConfig(fetched: FetchedData): AgentConfig {
  const graph = fetched.graph as Record<string, unknown>;
  const agentCfg = graph.agentConfig as Record<string, unknown> | undefined;
  return {
    systemPrompt: typeof agentCfg?.systemPrompt === 'string' ? agentCfg.systemPrompt : '',
    context: typeof agentCfg?.context === 'string' ? agentCfg.context : '',
    maxSteps: typeof agentCfg?.maxSteps === 'number' ? agentCfg.maxSteps : null,
  };
}
```

In `handleStreaming` and `handleNonStreaming`, check `ctx.fetched.appType`. If `'agent'`, call the agent loop path; otherwise, use existing workflow path. Extract the routing into the main handler:
...
Create `handleAgentStreaming` and `handleAgentNonStreaming` functions that:
1. Extract agent config from `ctx.fetched`
2. Create MCP session from graph's `mcpServers`
3. Call `executeAgentLoop` with callbacks that write SSE events
4. Persist results using the same persistence layer

**Important:** Keep each function under 40 lines. Split the agent execution path into separate helper functions.
...
The agent loop produces step-level data that needs to be persisted into `agent_execution_nodes` using `node_id = 'step-{N}'`. The existing `persistNodeVisits` function can be reused by mapping agent steps to the same format.

- [ ] **Step 1: Add agent step persistence helper**

Add a new exported function for persisting agent loop results. This maps each step's `ActionTokenUsage` entry (with `action: 'step-N'`) to a `saveNodeVisit` call:

```ts
interface AgentStepPersistenceParams {
  supabase: SupabaseClient;
  executionId: string;
  tokensLogs: CallAgentOutput['tokensLogs'];
  model: string;
}

export async function persistAgentSteps(params: AgentStepPersistenceParams): Promise<void> {
...
- [ ] **Step 2: Create a post-execution helper for agent results**

Add a `persistAgentPostExecution` function analogous to `persistPostExecution` but accepting `AgentLoopResult` instead of `CallAgentOutput`:

```ts
import type { AgentLoopResult } from '@daviddh/llm-graph-runner';

interface AgentPostExecutionParams {
  executionId: string;
  sessionDbId: string;
...
export async function persistAgentPostExecution(
  ...
  try {
    ...
    const totals = params.agentResult.totalTokens;
    await completeExecution(supabase, params.executionId, {
    ...
    refreshExecutionSummary(supabase).catch(() => { /* ignore */ });
  } catch (err) {
...
git commit -m "feat: add agent step persistence using step-N node IDs"
```
