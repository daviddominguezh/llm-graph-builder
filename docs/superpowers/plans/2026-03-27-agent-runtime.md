  ...
  for (const msg of responseMessages) {
    ...
    for (const part of msg.content) {
      if (part.type === 'tool-result') {
        outputMap.set(part.toolCallId, part.result);
      }
    }
  }
  return outputMap;
...
function appendResponseMessages(
  ...
  // Append the SDK's responseMessages directly — they already contain
  // the correct assistant (tool-call) and tool (tool-result) messages.
...
**Note:** The `generateText` call uses `maxSteps: 1` so the AI SDK executes exactly one round of tool calls per loop iteration — the SDK invokes the tools, collects results, and returns them in `response.messages` (exposed as `responseMessages` in our `LlmCallResult`). The outer `while` loop appends those `responseMessages` directly to the conversation (via `appendResponseMessages`) and calls the LLM again. We do **not** manually reconstruct assistant/tool messages — the SDK already produces correctly shaped `AssistantModelMessage` and `ToolModelMessage` objects. Tool outputs are extracted from `responseMessages` in `callAgentLlm` (via `buildToolOutputMap`) to populate each `AgentToolCallRecord.output` field, so tool results are available for callbacks and persistence. This gives us full control over per-step callbacks, persistence, and step-limit enforcement while letting the SDK handle actual tool invocation.
