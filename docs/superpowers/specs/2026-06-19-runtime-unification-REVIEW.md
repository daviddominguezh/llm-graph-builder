...
| `DeepReadonly` + ESLint-no-`as` as the immutability guarantee | 🟡 | Too weak alone (spread/clone/array-mutators bypass). Add deep `Object.freeze` at the ctx boundary. | |
| Per-mutation `simulation_state_patch` SSE + FE accumulation | 🟡 | Dual-ownership diverges on dropped patch/abort. Add sequence numbers + terminal full-state snapshot, or make the runtime the sole writer. | |
