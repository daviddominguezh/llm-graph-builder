...
| MCP tools fire real side effects in sim → badge | ✅ | **Decision:** MCP-sourced tools get a visible "real side effects" badge (discriminator `providerType === 'mcp'`) on the sim tool list **and** on tool-call cards in the sim view, with a tooltip — so users know MCP tools execute for real while builtins no-op. Ships **with** the no-op-builtins phase (§14 Phase 7), not after. Translations added (`simulation.mcpBadge.*`). Optional dry-run gate is a follow-up. Spec §12.4/§13. | |
| Forms + lead-scoring in simulation | ✅ | **Decision:** for now **all** builtin tools (forms + lead-scoring included) no-op in sim; bespoke per-tool sim behavior is deferred **uniformly for every tool**, to be built later. No special-casing. §2 goal reworded to match (no longer claims they're "wired in"). Note for the future work: lead-scoring already has `createSimulationStore`/`buildSimulationServices` (in-memory) to adapt; forms is from scratch. | |
...
| Composition tools get no sim guard | ✅ | **Fixed (§13):** all **four** composition tools (`create_agent` included) get no guard — they're dispatch sentinels / handled by recursion + `environment` propagation. Corrected two verified errors: `create_agent` writes **no DB record** (pure sentinel, `dispatchTools.ts` — moved out of the "synthetic-id guard" bucket), and `finish` is **child-only** (`buildTools.ts` gates on `ctx.isChildAgent`) — noted in §13 and §6.4. | |
