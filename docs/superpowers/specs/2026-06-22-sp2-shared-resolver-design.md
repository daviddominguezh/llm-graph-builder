...
- **R-a (was open (a)) — Regex. [CORRECTED post-implementation, commit 6ef355d0]** Canonical is `/\{\{(?<name>\w+)\}\}/gv` (named group, unicode-sets) — NOT the plain `/g` originally written here. The plain `/\{\{(\w+)\}\}/g` is **un-lintable** under this repo's ESLint (`prefer-named-capture-group` + `require-unicode-regexp`) and cannot pass without an eslint-disable (forbidden). The shipped `/gv` named form also matches the pre-existing `executeHelpers.ts:68` regex, so it is the *consistent* choice, not a divergence. Behaviorally identical for `\w` names (the `/v` flag doesn't change `\w`; parity is proven in the test suite); `extractTemplateVariables` reads `match.groups?.name`. **Do NOT revert to plain `/g` — it will fail lint.**
