-- SP4: restore per-tenant MCP config + variable_values/library_item_id.
--
-- Redefines public.restore_version_tx so reverting to a published version
-- (a) re-inserts graph_mcp_servers with variable_values + library_item_id (these
--     were silently dropped by the original body from 20260310000000), and
-- (b) repopulates graph_mcp_server_tenant_config from the snapshot's
--     'mcpTenantConfig' array ({ serverId, tenantId, variableValues }) added by
--     20260623100000.
--
-- Body copied verbatim from 20260310000000_add_restore_version_tx.sql; only the
-- MCP servers insert column list and a new per-tenant config restore step change.
-- graph_mcp_server_tenant_config rows for the agent are cleared transitively when
-- graph_mcp_servers is deleted (composite FK on delete cascade), so the new
-- inserts run after the servers are re-created to satisfy that FK.
...
begin
  ...
  -- they are not included in version snapshots and cannot be restored.
  -- Deleting graph_mcp_servers cascades graph_mcp_server_tenant_config rows.
  ...
  -- 6. Insert MCP servers (restore variable_values + library_item_id too)
  if v_snapshot->'mcpServers' is not null then
    ...
    loop
      insert into public.graph_mcp_servers (
        ...
        variable_values, library_item_id
      ) values (
        p_agent_id,
        v_server->>'id',
        v_server->>'name',
        ...
        coalesce(v_server->'variableValues', '{}'::jsonb),
        (v_server->>'libraryItemId')::uuid
      );
    end loop;
  end if;

  -- 7. Restore per-tenant MCP config (FK depends on graph_mcp_servers above)
  if v_snapshot->'mcpTenantConfig' is not null then
    for v_tenant_cfg in select * from jsonb_array_elements(v_snapshot->'mcpTenantConfig')
    loop
      insert into public.graph_mcp_server_tenant_config (
        agent_id, server_id, tenant_id, variable_values
      ) values (
        p_agent_id,
        v_tenant_cfg->>'serverId',
        (v_tenant_cfg->>'tenantId')::uuid,
        coalesce(v_tenant_cfg->'variableValues', '{}'::jsonb)
      )
      on conflict (agent_id, server_id, tenant_id) do update
        set variable_values = excluded.variable_values;
    end loop;
  end if;
